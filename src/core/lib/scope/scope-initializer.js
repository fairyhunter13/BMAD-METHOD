...
2. **Writing**: Use \`npx bmad-fh scope sync-up <scope>\` to promote artifacts
3. **Syncing**: Use \`npx bmad-fh scope sync-down <scope>\` to pull updates
...
npx bmad-fh workflow --scope ${scopeId}

# Check scope info
npx bmad-fh scope info ${scopeId}
\`\`\`
...
npx bmad-fh scope sync-up ${scopeId}

# Pull updates from shared layer
npx bmad-fh scope sync-down ${scopeId}
\`\`\`
