...
class ScopeManager {
  ...
  async createScope(scopeId, options = {}) {
    try {
      ...
      // Lock scopes.yaml during read-modify-write to prevent race conditions
      const scope = await this.stateLock.withLock(this.scopesFilePath, async () => {
        // Force reload from disk inside lock to avoid stale cache
        ...
        const scopeValidation = this.validator.validateScope(newScope, config.scopes);
        if (!scopeValidation.valid) {
          throw new Error(`Invalid scope configuration: ${scopeValidation.errors.join(', ')}`);
        }

        // Add to configuration
        config.scopes[scopeId] = newScope;
      ...
      // Create scope directory structure (outside lock — no config mutation)
      try {
        await this.initializer.initializeScope(scopeId, options);
      } catch (initError) {
        // Rollback: remove the scope from config to avoid orphaned entry
        try {
          await this.stateLock.withLock(this.scopesFilePath, async () => {
            const config = await this.loadConfig(true);
            delete config.scopes[scopeId];
            await this.saveConfig(config);
          });
        } catch {
          // Best-effort rollback — if this also fails, the orphaned entry stays
        }
        throw initError;
      }
  ...
  async updateScope(scopeId, updates = {}) {
    try {
      // Lock scopes.yaml during read-modify-write to prevent race conditions
      return await this.stateLock.withLock(this.scopesFilePath, async () => {
        // Force reload from disk inside lock to avoid stale cache
        const config = await this.loadConfig(true);
  ...
  async getScopePaths(scopeId) {
    try {
      ...
      const outputBase = config.settings?.default_output_base || '_bmad-output';
