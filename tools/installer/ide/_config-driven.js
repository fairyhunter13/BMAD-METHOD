...
class ConfigDrivenIdeSetup {
  constructor(platformCode, platformConfig) {
    ...
    // Set configDir from target_dir (or first target) so detect() works
    this.configDir = this.installerConfig?.target_dir || this.installerConfig?.targets?.[0]?.target_dir || null;
  }
  ...
  async setup(projectDir, bmadDir, options = {}) {
    ...
    if (this.installerConfig.targets && Array.isArray(this.installerConfig.targets)) {
  ...
  /**
   * Determine whether scope command should be installed to a given target.
   * Scope is a command — it should only go to targets that handle commands
   * (workflows, tasks, tools), NOT agents-only targets.
   * @param {string[]} [artifactTypes] - The artifact_types from config (undefined = no filter = install all)
   * @returns {boolean}
   */
  shouldInstallScopeToTarget(artifactTypes) {
    // undefined means "no filter" → install everything including scope
    if (!artifactTypes) return true;
    // Empty array means "skip everything"
    if (artifactTypes.length === 0) return false;
    // Scope is a command, only install if target handles non-agent types
    const commandTypes = new Set(['workflows', 'tasks', 'tools']);
    return artifactTypes.some((t) => commandTypes.has(t));
  }
  ...
  async installToTarget(projectDir, bmadDir, config, options) {
    const { target_dir, artifact_types } = config;
    const results = { agents: 0, workflows: 0, tasks: 0, tools: 0, scope: 0, skills: 0 };

    // Early return if artifact_types is an explicit empty array
    ...
    // Install scope command if this target handles commands
    if (this.shouldInstallScopeToTarget(artifact_types)) {
      const generator = new ScopeCommandGenerator(this.bmadFolderName);
      await generator.writeScopeCommand(targetPath);
      results.scope = 1;
    }
  ...
  /**
   * Install to multiple target directories (e.g. OpenCode: agents → /agent, commands → /command)
   * @param {string} projectDir - Project directory
   * @param {string} bmadDir - BMAD installation directory
   * @param {Object[]} targets - Array of target configurations
   * @param {Object} options - Setup options
   * @returns {Promise<Object>} Aggregated installation result
   */
  async installToMultipleTargets(projectDir, bmadDir, targets, options) {
    const aggregated = { agents: 0, workflows: 0, tasks: 0, tools: 0, scope: 0, skills: 0, skillDirectories: 0 };

    for (const target of targets) {
      const { results } = await this.installToTarget(projectDir, bmadDir, target, options);
      for (const key of Object.keys(aggregated)) {
        aggregated[key] += results[key] || 0;
      }
    }
  ...
  async printSummary(results, targetDir, options = {}) {
    ...
    const scopeNote = results.scope ? ' + scope command' : '';
    if (count > 0 || results.scope > 0) {
      await prompts.log.success(`${this.name} configured: ${count} skills${scopeNote} → ${targetDir}`);
    }
  }
  ...
  async cleanup(projectDir, options = {}) {
    ...
    // Clean target directory (single-target config)
    if (this.installerConfig?.target_dir) {
      await this.cleanupTarget(projectDir, this.installerConfig.target_dir, options);
    }

    // Clean target directories (multi-target config)
    if (this.installerConfig?.targets && Array.isArray(this.installerConfig.targets)) {
  ...
  async findAncestorConflict(projectDir) {
    const targetDir = this.installerConfig?.target_dir || this.installerConfig?.targets?.[0]?.target_dir;
