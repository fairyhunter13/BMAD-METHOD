...
const { clearCache } = require('./platform-codes');
...
describe('ConfigDrivenIdeSetup', () => {
  ...
  // =========================================================================
  // 7. IdeManager → scope wiring end-to-end (real platform-codes.yaml)
  // =========================================================================

  describe('IdeManager.setup() → bmad-scope.md wiring', () => {
    test('claude-code: IdeManager.setup() writes bmad-scope.md to .claude/skills', async () => {
      // Clear the module-level cache so this test always reads the real YAML
      clearCache();

      const manager = new IdeManager();
      await manager.ensureInitialized();

      const result = await manager.setup('claude-code', projectDir, bmadDir, { silent: true });

      expect(result.success).toBe(true);

      // The real platform-codes.yaml has target_dir: .claude/skills for claude-code
      const scopePath = path.join(projectDir, '.claude/skills', 'bmad-scope.md');
      expect(await fs.pathExists(scopePath)).toBe(true);
    });
  });

  // =========================================================================
  // 8. ScopeCommandGenerator content correctness
  // =========================================================================
  ...
  // =========================================================================
  // Multi-scope feature: installer runtime: true and agent SKILL scope step
  // =========================================================================

  describe('Fix 2: Installer skips runtime: true variables', () => {
    test('collectModuleConfig skips scope (runtime: true, no prompt)', async () => {
      const { OfficialModules } = require('../modules/official-modules');
      const instance = new OfficialModules();
      instance.allAnswers = {};
      instance.skipPrompts = true;

      // Build a synthetic module.yaml-like object with a runtime-flagged variable
      const moduleConfig = {
        scope: {
          default: '',
          result: '{value}',
      ...
      // Simulate the key-filtering logic used in collectModuleConfig
      const runtimeKeys = Object.keys(moduleConfig).filter((key) => {
        const item = moduleConfig[key];
        return item && typeof item === 'object' && item.runtime === true;
      });

      const staticKeys = Object.keys(moduleConfig).filter((key) => {
        const item = moduleConfig[key];
        return item && typeof item === 'object' && !item.prompt && item.result && !item.runtime;
      });

      // Runtime keys should NOT appear in static keys (they are skipped)
      expect(runtimeKeys).toContain('scope');
      expect(runtimeKeys).toContain('scope_path');
      expect(runtimeKeys).toContain('scope_tests');
      expect(staticKeys).not.toContain('scope');
      expect(staticKeys).not.toContain('scope_path');
      expect(staticKeys).not.toContain('scope_tests');
    });

    test('collectModuleConfigQuick newStaticKeys excludes runtime: true items', async () => {
      // Verifies the filter condition used in collectModuleConfigQuick
      const moduleConfig = {
        scope: { default: '', result: '{value}', runtime: true },
        scope_path: { default: '{output_folder}', result: '{value}', runtime: true },
        scope_tests: { default: '{scope_path}/tests', result: '{project-root}/{value}', runtime: true },
        planning_artifacts: {
          prompt: 'Where should planning artifacts be stored?',
          default: 'docs/planning',
          result: '{project-root}/{value}',
      ...
      const existingKeys = []; // simulate fresh install (nothing existing)

      const newStaticKeys = Object.keys(moduleConfig).filter((key) => {
        const item = moduleConfig[key];
        return item && typeof item === 'object' && !item.prompt && item.result && !item.runtime && !existingKeys.includes(key);
      });

      // scope, scope_path, scope_tests must NOT be in newStaticKeys
      expect(newStaticKeys).not.toContain('scope');
      expect(newStaticKeys).not.toContain('scope_path');
      expect(newStaticKeys).not.toContain('scope_tests');
      // planning_artifacts has a prompt so it also won't be a static key
      expect(newStaticKeys).toHaveLength(0);
    });
  });

  describe('Fix 1: Agent SKILLs contain scope resolution step', () => {
    ...
    for (const skillPath of agentSkills) {
      test(`${skillPath} contains "Resolve active scope"`, () => {
        const fullPath = path2.join(repoRoot, skillPath);
        const content = fs2.readFileSync(fullPath, 'utf8');
        expect(content).toContain('Resolve active scope');
      });
    }
  });
});
