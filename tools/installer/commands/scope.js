...
const { select, text, confirm, isCancel } = require('../prompts');

// Import help functions from separate module
const { showHelp, showSubcommandHelp, configureCommand, getHelpText } = require('../scope-help');
...
async function handleCreate(projectRoot, scopeId, options) {
  ...
  if (!name) {
    const inputName = await text({
      ...
      default: scopeId.charAt(0).toUpperCase() + scopeId.slice(1).replaceAll('-', ' '),
    });
  ...
  await manager.createScope(scopeId, {
    name,
    description,
...
async function handleSyncUp(projectRoot, scopeId, options) {
  ...
  try {
    const syncOptions = {
      force: options.resolution === 'keep-local',
    };
 ...
 */
async function handleSet(projectRoot, scopeId, _options) {
  const manager = new ScopeManager({ projectRoot });
  const scopeFilePath = path.join(projectRoot, SCOPE_FILE_NAME);
